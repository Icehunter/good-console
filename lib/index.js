...
internals.utility = {
  ...
  formatResponse(event, tags, settings) {
    const headers = event.headers ? JSON.stringify(event.headers) : '';
    const requestPayload = event.requestPayload ? JSON.stringify(event.requestPayload) : '';
    const responsePayload = event.responsePayload ? JSON.stringify(event.responsePayload) : '';
    ...
    const outputs = [
      `${event.instance}: ${method} ${event.path} ${statusCode} (${event.responseTime}ms)`
    ];

    Object.keys(stringified).forEach((key) => {
      if (settings[key]) {
        outputs[outputs.length] = stringified[key];
      }
    });
